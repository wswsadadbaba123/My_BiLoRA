...
class Attention_LoRA(nn.Module):
    ...
    def forward(self, x, task, register_hook=False, get_feat=False,get_cur_feat=False):
        ...
        #此处直接改了关于task的行为
        if task > -0.5:
            weight_k = torch.stack([torch.mm(self.lora_B_k[t].weight, self.lora_A_k[t].weight) for t in range(1)], dim=0).sum(dim=0)
            weight_v = torch.stack([torch.mm(self.lora_B_v[t].weight, self.lora_A_v[t].weight) for t in range(1)], dim=0).sum(dim=0)
